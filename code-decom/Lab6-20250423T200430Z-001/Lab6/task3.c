...
void qs(int Ls[], int lo, int hi){
	if (lo < hi){
		int p = Sselect(Ls, lo, hi); // Ls[p] is the pivot
...
void qs2(int Ls[], int lo, int hi){
	if (lo < hi){
		int p = qselect(Ls, lo, hi); // Ls[p] is the pivot
		p = part(Ls, lo, hi, p); // Ls[p] is the pivot
		qs(Ls, lo, p - 1);
		qs(Ls, p + 1, hi);
	}
...
void helper(int numbers[], char* fname, int n) {
    ...
    printf("select partition take %f seconds\n", time_taken);
    ...
    printf("qselect partition take %f seconds\n", time_taken);
}
